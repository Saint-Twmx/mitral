...
def crop_to_nonzero_new(data, seg=None, nonzero_label=-1):
...
class ImageCropper(object):
    ...
    @staticmethod
    def crop_new(data, properties, seg=None):
        data, seg, bbox = crop_to_nonzero_new(data, seg, nonzero_label=-1)
        properties["crop_bbox"] = bbox
        # properties["classes"] = np.unique(seg)
        # import pandas as pd
        # properties["classes"] = np.sort(pd.unique(seg.ravel()))  # 为了加速
        properties["classes"] = np.array([-1, 0])  # 为了加速
        seg[seg < -1] = 0
        properties["size_after_cropping"] = data[0].shape
        return data, seg, properties
    ...
    @staticmethod
    def crop_from_file(data_file, seg_file=None, new_c = False):
        properties = OrderedDict()
        # data, seg, properties = load_case_from_list_of_files(data_files, seg_file)
        ...
        if new_c: # 为真，返回我修改的
            return ImageCropper.crop_new(data, properties, seg)
        else:
            return ImageCropper.crop(data, properties, seg)
