...
from nnunet_sub.postprocessing.consolidate_postprocessing import (
    consolidate_folds,
    collect_cv_niftis,
)
        ...
        for k, v in results.items():
            if v == best:
                ...
                if k.startswith("ensemble"):
                    ...
                    predict_str += (
                        "nnUNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr "
                        + tr
                        + " -ctr "
                        + trc
                        + " -m "
                    ...
                    predict_str += (
                        "nnUNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL2 -tr "
                        + tr
                        + " -ctr "
                        + trc
                        + " -m "
                    ...
                    if not disable_postprocessing:
                        predict_str += (
                            "nnUNet_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER -pp "
                            + join(
                                network_training_output_dir,
                                "ensembles",
                                id_task_mapping[t],
