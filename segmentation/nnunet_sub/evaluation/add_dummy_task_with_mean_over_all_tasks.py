    ...
    if task != "Task999_ALL":
        ...
        if name in task_descriptors:
            if task not in list(mean_scores[name].keys()):
                ...
                raise RuntimeError(
                    "duplicate task %s for description %s" % (task, name)
                )

for t in task_ids_with_no_stage0:
    mean_scores["3D stage0 final 2, fold0"][t] = mean_scores[
        "3D fullres final 2, fold0"
    ][t]
...
for i in mean_scores.keys():
    try:
        for t in list(a):
            assert (
                t in mean_scores[i].keys()
            ), "did not find task %s for experiment %s" % (t, i)
        new_res = OrderedDict()
        new_res["name"] = i
